...
_.=                                    /*placeholder for the array of _s*/
--stuff='1e9 50 5 40 4 1 100 30 3 12 2 8 9 7 6 6 10 20 0' /*alphabetically*/
stuff='50 5 40 4 1 100 30 3 12 2 8 9 7 6 6 10 20 0' /*alphabetically*/
...
do j = 1 for N                         /*let's start to boogie-woogie.  */
  _.j = word(numbers, j)               /*plug in one number at a time.  */
  if datatype(_.j, 'Numeric') then _.j = _.j / 1  /*normalize it if num.*/
...
m = max(length(_.1),length(_.N))       /*width of smallest | largest num*/
say
say 'after sort:'
indent = left('', 20)                        /*same as:  COPIES(' ',20) */
                                       /*∙∙∙but LEFT pads  [much faster]*/
do k = 1 for N                         /*list (sorted) array's elements.*/
  say indent 'array element' right(k, w) '───>' right(_.k, m)
  end k
exit                                   /*stick a fork in it, we're done.*/
/*───────────────────────────────────SortItem subroutine────────────────*/
sortItem:
procedure expose _.
parse arg ?                                         /*sorts single item.*/
  do Asort = 1  until \switched                     /*cook until cooked.*/
    switched=0                                      /*hunky-dorey so far*/
    do i = 1 while _.i \== '' & \switched
      if _.? >= _.i then iterate                           /*this one ok*/
        parse value   _.?  _.i     with     _.i  _.?
...
inOrder:
procedure expose _.
parse arg howMany                                  /*is array in order? */
do m = 1 for howMany - 1
  next = m + 1
  if _.m > _.next then return 0
