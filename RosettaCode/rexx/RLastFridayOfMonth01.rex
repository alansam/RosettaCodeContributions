...
S_ = " Monday TUesday Wednesday THursday Friday SAturday SUnday"
B_ = " JAnuary February MARch APril MAY JUNe JULy AUgust September",
     " October November December"
--upper S_ B_                                            /*uppercase strings*/
S_ = upper(S_)
B_ = upper(B_)
...
dw = pos(' 'dow, S_)                                   /*find  day-of-week*/
if dw == 0                     then call .er 'is invalid:', 1
if dw \== lastpos(' 'dow, S_)  then call .er 'is ambigious:', 1

if datatype(mm, 'month') then                          /*if MM is alpha...*/
do
  m = pos(' 'mm, B_)                                   /*maybe its good...*/
  if m == 0                    then call .er 'is invalid:', 1
  if m \== lastpos(' 'mm, B_)  then call .er 'is ambigious:', 2
  mm = wordpos(word(substr(B_, m), 1), B_) - 1         /*now, use true Mon*/
  end
...
tdow = wordpos(word(substr(S_, dw), 1), S_) - 1        /*target DOW, 0-->6*/
                                                       /*day# of last dom.*/
U_ = date('B', right(yy + (mm = 12), 4)right(mm // 12 + 1, 2, 0)"01", 'S') - 1
Q_ = U_ // 7                                           /*calc. DOW,  0-->6*/
if Q_ \== tdow then U_ = U_ - Q_ - 7 + tdow + 7 * (Q_ > tdow) /*not DOW?  Adjust.*/
return date('weekday', U_, "B") date(, U_, 'B')        /*return the answer*/

.er:
arg , U_
say
say '***errorB_*** (in LASTDOW)'
say /*tell error,  and */
say word('day-of-week month year excess', arg(2)) arg(1) a.U_
say
exit 13                                       /*... then exit.   */
