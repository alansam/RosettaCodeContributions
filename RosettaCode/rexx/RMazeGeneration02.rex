...
If imax = '' | imax = '.' Then imax = 10
If jmax = '' | jmax = '.' Then jmax = 15
If seed = '' | seed = '.' Then seed = 4711
dD = '\' /* DOS path delimiter */
uD = '/' /* UNIX path delimiter */
parse source . . ss
ss = translate(ss, uD, dD)
pd = lastpos(uD, ss)
parse var ss . +(pd) ss '.' . /* get the filename; drop any extensions */
say ss imax jmax seed
