...
ss=!||'80'x||copies('0'x,plus%8-1)reverse(right(d2c(8*hh),4,'0'x))||'00000000'x

  do j=0 to length(ss)%64-1             /*process the message.           */
  a_=a;  b_=b;  c_=c;  d_=d
  chunk=j*64

     do k=1 for 16                     /*process the message in chunks. */
     !.k=reverse(substr(ss,chunk+1+4*(k-1),4))
     end
...
.lR: procedure; parse arg _,hh; if hh==0 then return _      /*left rotate.*/
?=x2b(c2x(_)); return x2c(b2x(right(?||left(?,hh),length(?))))
