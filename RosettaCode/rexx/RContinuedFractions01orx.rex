...
a='';  a_='';  b=''                           /*omitted ß terms are assumed = 1*/
/*══════════════════════════════════════════════════════════════════════*/
...
/*══════════════════════════════════════════════════════════════════════*/
T=10000
a=1;  do j=1 for T;        a=a 1/j; a_=a_ '1/'j; end;     call tell '½π, ½pi'
/*══════════════════════════════════════════════════════════════════════*/
T=10000
a=0 1 rep(2)
...
/*────────────────────────────────CF subroutine─────────────────────────*/
cf: procedure;  parse arg C x,y;      i_=0;      numeric digits  digits()+5
           do k=words(x) to 1 by -1;   a=word(x,k);  b=word(word(y,k) 1,1)
           d=a+i_;  if d=0 then call divZero  /*in case divisor is bogus.*/
           i_=b/d                             /*here's a binary mosh pit.*/
           end   /*k*/
return i_+C
/*────────────────────────────────DIVZERO subroutine────────────────────*/
divZero: say;  say '***error!***';  say 'division by zero.';  say; exit 13
/*────────────────────────────────GETT subroutine───────────────────────*/
getT: parse arg stuff,width,ma,mb,u_
                do m=1; mm=m+ma; mn=max(1,m-mb);   w=word(stuff,m)
                w=right(w,max(length(word(As,mm)),length(word(Bs,mn)),length(w)))
                if length(u_ w)>width  then leave   /*stop getting terms?*/
                u_=u_ w                              /*whole, don't chop. */
                end   /*m*/                        /*done building terms*/
return strip(u_)                                    /*strip leading blank*/
/*────────────────────────────────REP subroutine────────────────────────*/
...
/*────────────────────────────────TELL subroutine───────────────────────*/
tell: parse arg ?; v=cf(a,b); numeric digits showdig; As=rf(a_ a); Bs=rf(b)
