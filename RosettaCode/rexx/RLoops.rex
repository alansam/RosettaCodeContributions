...
procedure
  ...
  loop line = 1 to out.0
    say (out.line~strip())~strip('t', ',')'!'
    end line
...
procedure
  ...
  loop i1 = 1 for dim2~dimension(1)
    loop i2 = 1 for dim2~dimension(2)
      dim2[i1, i2] = random(20) + 1
      end i2
    end i1

  loop x1 = 1 for dim2~dimension(1)
    out.0 = x1
    loop x2 = 1 for dim2~dimension(2)
      out.x1 ||= dim2[x1, x2]~right(3) || ' '
      if dim2[x1, x2] = 20 then leave x1
      end x2
    end x1

    loop line = 1 to out.0
      say line~right(4)': ' || out.line~strip('t')
      end line
