...
do j = 1 for n
  say 'item' right(j, w) "after the radix sort:" right(A_.j, width + 1)
  end j
exit                                   /*stick a fork in it, we're done.*/
/*-----------------------------------RADSORT subroutine-----------------*/
radSort:
procedure expose A_. width
  parse arg size
  mote = c2d(' ')
  H_ = 1
  !.H_._b = 1
  ...
  do i = 1 for size
    y = A_.i
    A_.i = right(abs(y), width, 0)
    if y < 0 then A_.i = '-'A_.i
    end i
/*======================================where the rubber meets the road.*/
  ...
  if L >> H then iterate
  _ = ''
  if L == H then if ctr._ == 0 then do
    H_ = H_ + 1
    !.H_._b = low
    !.H_._n = n
    !.H_._i = radi + 1
  ...
  do until i == max
    ...
    d = ctr._
    if d <= mote then do
      if d > 1 then call .radSortP top._, d
      iterate
      end
      H_ = H_ + 1
      !.H_._b = top._
...
.radSortP:
  parse arg bbb,nnn
  do k = bbb + 1 for nnn - 1
    q = A_.k
    do j = k - 1 by -1 to bbb while q << A_.j
      jp = j + 1
      A_.jp = A_.j
      end
    jp = j + 1
