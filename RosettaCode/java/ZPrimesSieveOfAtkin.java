 ...
 */
public class ZPrimesSieveOfAtkin {
   ...
   * @param limit
   */
  public ZPrimesSieveOfAtkin(int limit) {
    setLimit(limit);
    setSieve(new BitSet(getLimit() + 1));
    return;
  }
  ...
  public static void main(String[] args) {
    ...
    ZPrimesSieveOfAtkin primes;
    primes = new ZPrimesSieveOfAtkin(inLim);
  ...
  public void seive() {
    ...
    getSieve().clear(0); // false;
    getSieve().clear(1); // false;
    getSieve().set(2); // true;
    getSieve().set(3); // true;
    ...
    for (int x_ = 1; x_ <= getLimitSqrt(); x_++) {
      for (int y_ = 1; y_ <= getLimitSqrt(); y_++) {
        ...
        if (n_ <= getLimit() && (n_ % 12 == 1 || n_ % 12 == 5)) {
          getSieve().set(n_, !getSieve().get(n_));
        }
   ...
   * @return the seive
   */
  protected void setSieve(BitSet seive) {
    this.sieve = seive;
    return;
  }
