...
int main()
{
  ...
  rfunc rfuncs[] = { f1, f2, f3, f3 };
  rfunc rfuncsA[] = { f1a, f2a, f3a, f3a };
  double ivals[] = {
    0.0, 1.0,
    1.0, 100.0,
    0.0, 5000.0,
  ...
  for (j = 0; j < (sizeof(rfuncs) / sizeof(rfunc)); j++)
  {
    for (i = 0; i < 5 ; i++)
    {
      ic = (*pfuncs[i])(ivals[2 * j], ivals[2 * j + 1], approx[j], rfuncs[j]);
      printf("%10s [ 0,1] num: %+lf, an: %lf\n", names[i], ic, INTG((* rfuncsA[j]), ivals[2 * j], ivals[2 * j + 1]));
    }
    printf("\n");
  }
}
