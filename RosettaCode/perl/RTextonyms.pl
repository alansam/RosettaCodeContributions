...
# Program title.
#
# (C) Copyright Alan Sampson 2015.  All rights reserved.
# ------------------------------------------------------------------------------
#
# %Z%%M% %I% %W% %G% %U%
# @(#) RTextonyms.pl - $Header: $
# @(#) RTextonyms.pl - $Id: $
#
...
# ---------+------------+-----+-------------------------------------------------
#          | 2015-02-18 | AS  | Initial development.
#          |            |     |
# ---------+------------+-----+-------------------------------------------------
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
sub find_in_dictionary {
	my @m = qw/$ $ abc def ghi jkl mno pqrs tvu wxyz/;
	(my $r = shift) =~ s{(\d)}{[$m[$1]]}g;
	grep /^$r$/i, split ' ', `cat unixdict.txt`; # cats don't run on windows
}
